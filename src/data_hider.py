...
def hide_data_in_image():
    print("[Info] Hiding data in image...")
    
    image_path = input("Enter the image path to hide data in: ").strip()
    if not os.path.isfile(image_path):
        print("[Error] The image file does not exist.")
        return
    
    # Prompt user for data to hide
    data_to_hide = input("Enter the data you want to hide in the image: ").strip()
    
    # Hide data using LSB (Least Significant Bit) encoding
    output_image_path = "hidden_" + os.path.basename(image_path)
    lsb.hide(image_path, data_to_hide).save(output_image_path)
...
def extract_data_from_image():
    ...
    image_path = input("Enter the image path with hidden data: ").strip()
    if not os.path.isfile(image_path):
        print("[Error] The image file does not exist.")
        return
